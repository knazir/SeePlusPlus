...
**Credential flow:** Secrets Manager holds a JSON secret with `username`/`password`; `AWS::SecretsManager::SecretTargetAttachment` injects `host`/`port`/`dbname` after the DB is created. Backend's task role has `secretsmanager:GetSecretValue` via the addon's ManagedPolicy (Copilot auto-attaches any output suffixed `AccessPolicy`). On boot, `backend/src/db.ts` reads `WORKSPACES_DB_SECRET_ARN`, fetches the JSON, assembles `DATABASE_URL`. Locally, docker-compose sets `DATABASE_URL` directly and the secret path is skipped.
